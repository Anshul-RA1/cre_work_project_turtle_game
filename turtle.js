...
class Player{
   constructor(gameWidth, gameHeight){
    ...
    this.image = document.getElementById('tutrleImage');
    this.frameX = 0;
    this.maxFrame = 1;
    this.fps = 5;
    this.frameTimer = 0;
...
// adding backgorund music
bgSound = new Audio("Hostiles-Inbound-Epic-Battle.mp3");
bgSound.loop = true;
bgSound.play();
...
function deisplayStatus(context){
  ...
  if(gameOver){
    ...
    context.fillStyle = 'black';
    context.fillText('GAME OVER', canvas.width/2 + 2, 150);
...
const retryGame = this.addEventListener('click', retry());

function retry(context){
  if(gameOver){
    context.textAlign = 'center';
    context.fillStyle = 'black';
    context.fillText('Retry', canvas.width/2, 300);
    context.fillStyle = 'black';
    context.fillText('Retry', canvas.width/2 + 2, 300);
